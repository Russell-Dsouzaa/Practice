...
var rotate = require('../../189.RotateArray/Javascript/RotateArray.js');
...
function main() {
    var nums = [], size = Number(prompt("Enter the size of the array : ")), k = Number(prompt("Enter the number of spots by which to left-shift the array elements : ")), target = Number(prompt("Enter the target element : "));
    for (var i = 0; i < size; i++) {
        nums.push(Number(prompt("Enter the ".concat(i + 1, "th element of the array : "))));
    }
    ...
    rotate(nums, k);
    var result = search(nums, target); // 'nums' here isn't the same as user entered. It has been rotated permanently(original array has been modified instead of a copy)
    if (result === -1) {
        console.log("".concat(target, " isn't present in ").concat(nums));
    }
    else {
        console.log("".concat(target, " is present at index ").concat(result, "(0-indexed)"));
    }
}
main();
/*
