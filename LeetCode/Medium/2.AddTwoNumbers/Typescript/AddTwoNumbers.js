...
var _a = require('../../2058.FindTheMinimumAndMaximumNumberOfNodesBetweenCriticalPoints/Javascript/FindTheMinimumAndMaximumNumberOfNodesBetweenCriticalPoints'), Node = _a.Node, LinkedList = _a.LinkedList;
function addTwoNumbers(LL1, LL2) {
    var LL3 = new LinkedList(), num1 = 0, num2 = 0, curr1 = LL1.head, curr2 = LL2.head;
    while (curr1) {
        num1 *= 10;
        num1 += curr1.val;
        curr1 = curr1.next;
    ...
    var num3 = num1 + num2;
    if (num1 === 0 && num2 === 0) {
        var newNode = new Node(); // we don't pass in the value of '0' since the way Node() is setup, it already initializes new nodes to the value of 0 and points to null by default
    }
    while (num3 > 0) {
        var newNode = new Node(num3 % 10);
        if (!LL3.head) {
            LL3.head = newNode;
        }
        else {
...
function main() {
    var num1 = Number(prompt("Enter the first number : ")), num2 = Number(prompt("Enter the second number : ")), LL1 = new LinkedList(), LL2 = new LinkedList();
    while (num1 > 0) {
        var temp = new Node(num1 % 10);
        if (!LL1.head) {
            LL1.head = temp;
        }
        else {
...
Learnings :-
    ...
    1. At LOC 4, we could've used 'const var_name = require(); too. But then we would have to call those classes with the var_name such
