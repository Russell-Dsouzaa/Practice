...
function rotate(nums, k) {
    k = k % nums.length; // this is to take care of the condition where the input array is smaller in size than the value of k (i.e. nums.length < k)
    var result = [], j = 0;
    for (var i = 0; i < nums.length - k; i++) {
        result[i + k] = nums[i]; // initially, I was getting the error "cannot assign type 'number' to type 'never', then after reading on StackOverflow, I realised that i didn't define type to 'result' array
    }
...
function main() {
    var nums = [], k = Number(prompt("Enter the value for 'k' : ")), size = Number(prompt("Enter the size of the array : "));
