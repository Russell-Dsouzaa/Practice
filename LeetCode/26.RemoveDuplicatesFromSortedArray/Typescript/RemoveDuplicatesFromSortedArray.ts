import * as PromptSync from 'prompt-sync';
const prompt = PromptSync({sigint : true});

function rmDuplicates(arr1 : number[]) : number[] | number { // we type anootated the function's return type to 'either an integer or an integer array', this is because I initially wanted to return the entire array(original array)
    if (arr1.length < 2) return arr1;

    var k = 0;

    for (let i : number = 0;i < arr1.length;i++) {
        while (i < arr1.length - 1 && arr1[i] === arr1[i + 1]) {
            i++;
        }

        arr1[k++] = arr1[i];
    }

    return k;
}

function main() : void {
    var size : number = Number(prompt("Enter the size of the array : ")), arr1 : number[] = [];

    for (let i : number = 0;i < size;i++) {
        arr1.push(Number(prompt(`Enter the ${i + 1}th element of the array : `)));
    }

    var result : number | number[] = rmDuplicates(arr1);
    console.log(result);
}

main();