...
function main() {
    ...
    for (var i = 0; i < size; i++) {
        arr.push(Number(prompt("Enter the ".concat(i + 1, "th element of the array(Enter them in ascending order) : "))));
    }
    var result = search(arr, target);
    if (presence) {
        console.log("".concat(target, " is present in the input array at ").concat(result, " index(0-indexed)"));
    }
    else {
        console.log("".concat(target, " is not present in the input array, it would be present at ").concat(result, " index"));
    }
}
main();
/*
