...
function numOfPairsWithAbsDiff(arr1 : number[], k : number) : number {
    var count : number = 0;
...
function main() {
    ...
    var result : number = numOfPairsWithAbsDiff(array1,k);  // I know (baap ko mat sikha), I didn't call the function directly within the console.log() function because I wanted to type-check the function's return value
...
/*
    1. At LOC 19. Saw it in a Primeagen video(as one does), that in TS & JS, there's two ways to assign type to a variable, implicit and explicit. Implicit is where we tell the compiler what datatype we're 
