...
function MAD(arr1) {
    let result = [], sortedArray = arr1.sort((a,b) => a - b), minDist = Number.MAX_VALUE;  // we do MAX_VALUE assuming that the minimum distance could be 0 (we could've taken any -ve value as well, since absolute distances are calculated in positive values)
...
function main() {
    let size = Number(prompt("Enter the size of the input array : ")), array1 = [];

    for (let i = 0;i < size;i++) {
        array1.push(Number(prompt(`Enter the ${i + 1}th element of the input array : `)));
    }

    let result = MAD(array1);
...
Learnings :-
    ...
    3. At LOC 29, Initially we printied the result as follows console.log(`answer test ${result}`); , now this logged the resulting array in the console as follows : [1,2,2,3,3,4] instead of the 
